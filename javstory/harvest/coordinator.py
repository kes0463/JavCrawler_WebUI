    ...
    try:
        ...
        try:
            ...
            with get_db_session_ctx() as session:
                ...
                if row:
                    ...
                    meta_title_bad = _is_boilerplate_title(title_ko) or _is_boilerplate_title(title_ja_db)
                    if meta_title_bad and not force_rebuild_story_context:
                        needs_crawling = True
                        needs_translation = True
                        log_ts(
                            f"⚠️ {code} 제목이 리다이렉트/이적 안내 문구로 보여 웹 수집을 다시 시도합니다."
                        )

                    if has_ja_full and not force_rebuild_story_context and not meta_title_bad:
        ...
        if needs_crawling or force_rebuild_story_context:
            ...
            if _is_boilerplate_title(original_title):
                original_title = raw_title if not _is_boilerplate_title(raw_title) else ""
        ...
        if (needs_translation or force_rebuild_story_context) and not skip_translation:
            ...
            if (
                ...
                scrape_title_ko = raw_title
                ja_candidate = (original_title or "").strip()
