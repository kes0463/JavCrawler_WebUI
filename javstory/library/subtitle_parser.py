"""Subtitle discovery and parsing — shared by QML player and WebUI."""
...
def _split_ass_comma_fields( body: str) -> list[str]:
...
def _ass_drawing_to_svg( raw: str, scale_exp: int) -> tuple[str, tuple[float, float, float, float]]:
...
def _parse_ass_overrides( block: str, state: dict, styles: dict, default_style: str) -> None:
    ...
    for m in token_re.finditer(residual):
        ...
        if tag in ("c", "1c"):
            ...
            a = _ass_alpha_to_byte(arg)
            if a is not None:
                state["primary_alpha"] = a
                state["outline_alpha"] = a
                state["shadow_alpha"] = a
        elif tag == "1a":
            state["primary_alpha"] = _ass_alpha_to_byte(arg)
        elif tag == "3a":
            state["outline_alpha"] = _ass_alpha_to_byte(arg)
        elif tag == "4a":
            state["shadow_alpha"] = _ass_alpha_to_byte(arg)
        elif tag == "fn":
            ...
            if style:
                _apply_style_to_state(state, style)

def _apply_style_to_state( state: dict, style: dict) -> None:
...
def _parse_ass( text: str) -> list:
    ...
    play_res_y = 288
    wrap_style = 0
    styles: dict[str, dict] = {"Default": _ass_default_style()}
    ...
    for raw_line in text.splitlines():
        ...
        if section == "events":
            ...
            start_ms = _parse_ass_timestamp(event.get("start", ""))
            end_ms = _parse_ass_timestamp(event.get("end", ""))
            if end_ms <= start_ms:
                continue
            style_name = (event.get("style", "") or "Default").strip() or "Default"
            style = styles.get(style_name) or styles.get("Default") or _ass_default_style()
            margin_l = _safe_int(event.get("marginl", "0")) or _safe_int(style.get("MarginL", 10)) or 10
            margin_r = _safe_int(event.get("marginr", "0")) or _safe_int(style.get("MarginR", 10)) or 10
            margin_v = _safe_int(event.get("marginv", "0")) or _safe_int(style.get("MarginV", 10)) or 10
            ...
            for seg in segments:
                line = _build_ass_line(seg, styles, style_name, margin_l, margin_r, margin_v)
                if line is None:
                    continue
...
def _build_ass_line(
    ...
    style = styles.get(default_style) or _ass_default_style()
    _apply_style_to_state(state, style)
    ...
    while i < len(segment):
        ...
        if ch == "{":
            ...
            _parse_ass_overrides(block, state, styles, default_style)
        ...
        if not run_text:
            continue
        if state.get("draw"):
            path, bbox = _ass_drawing_to_svg(run_text, int(state.get("draw") or 1))
            if not path:
                continue
            ...
            runs.append({
                ...
                "fill": _ass_color_to_argb(state["primary_value"], state.get("primary_alpha")),
                "stroke": _ass_color_to_argb(state["outline_value"], state.get("outline_alpha")),
            ...
            runs.append({
                ...
                "primary": _ass_color_to_argb(state["primary_value"], state.get("primary_alpha")),
                "outline": _ass_color_to_argb(state["outline_value"], state.get("outline_alpha")),
                "shadow": _ass_color_to_argb(state["shadow_value"], state.get("shadow_alpha")),
